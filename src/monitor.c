...
/*   Updated: 2024/04/27 14:47:48 by pclaus           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/philosopers.h"

/*This function spinlocks untill all threads are running because it waits for all the threads to run*/
...
static bool	philosopher_has_died(t_philosopher *philosopher)
{
	...
	time_elapsed = get_time(MILLISECOND)
		- get_long(&philosopher->philosopher_mutex,
			&philosopher->time_since_last_meal);
...
void	*monitor_dinner(void *table)
{
	...
	while (!all_threads_running(&data->data_mutex, &data->nb_of_threads_running,
		...
		;
	/*To make sure the time since a philosopher has eaten is less than the actual time to die. It's like a referee checking on all philosophers*/
